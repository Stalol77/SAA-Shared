...
function EFFECT:Init(data)
	...
	local flashCount = math.Round(self.FlashSize * 8)
	    ...
	    for _ = 5, 9 do
		    local particle = emitter:Add("particle/water/waterdrop_001a", self.vOffset + FrameTime() * AddVel)

		    if (particle) then
			    particle:SetVelocity(dir * 6 * self.FlashSize + 1.05 * AddVel)
			    particle:SetLifeTime(0)
			    particle:SetDieTime(self.Life * 1)
			    particle:SetStartAlpha(math.Rand(15, 40))
			    particle:SetEndAlpha(0)
