...
function EFFECT:Init(data)
	...
	local flashCount = math.Round(self.FlashSize * 8)
	    ...
	    for _ = 1, flashCount do
			local particletable = { "effects/muzzleflashx_nemole_w", "effects/scotchmuzzleflashw" }
		    local particle = emitter:Add( particletable[math.random( 1, #particletable)], self.vOffset + FrameTime() * AddVel)

		    if (particle) then
			    particle:SetVelocity(dir * 6 * self.FlashSize + 1.05 * AddVelgas)
			    particle:SetLifeTime(0)
			    particle:SetDieTime(self.Life * 1.3)
			    particle:SetStartAlpha(math.Rand(0, 150))
			    particle:SetEndAlpha(0)
			    particle:SetStartSize(1.5 * math.Rand(0.8, 1.5) * self.FlashSize)
			    particle:SetEndSize(100 * math.Rand(0.5, 1) * self.FlashSize)
	    ...
	    for _ = 50, 50 do
			local path = "particle/smokesprites_00"
			local particletable = { path .. "07", path .. "08", path .. "09", path .. "10", path .. "11", path .. "12", path .. "13", path .. "14", path .. "15", path .. "16",}
		    local particle = emitter:Add( particletable[math.random( 1, #particletable)], self.vOffset + FrameTime() * AddVel)
		    ...
		    if (particle) then
			    ...
			    particle:SetStartAlpha(math.Rand(0,50))
			    particle:SetEndAlpha(0)
			    particle:SetStartSize(1.5 * math.Rand(0.8, 1.5) * self.FlashSize)
	    ...
	    for _ = 0, 0 do
			local particletable = { "effects/muzzleflashx_nemole", "effects/scotchmuzzleflash4" }
		    local particle = emitter:Add( particletable[math.random( 1, #particletable)], self.vOffset + FrameTime() * AddVel)
			local alphatable = { 255, 0 }
		    if (particle) then
			    particle:SetVelocity(dir * 6 * self.FlashSize + 1.05 * AddVel)
			    particle:SetLifeTime(0)
			    particle:SetDieTime(self.Life * 0.25)
			    particle:SetStartAlpha( alphatable[math.random(1, #alphatable)])
			    particle:SetEndAlpha(alphatable[math.random(1, #alphatable)])
				particle:SetColor(2, 2, 1)
			    particle:SetStartSize(10 * math.Rand(1, 1.5) * self.FlashSize)
			    particle:SetEndSize(10 * math.Rand(0.5, 1) * self.FlashSize)
