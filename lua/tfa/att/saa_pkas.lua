...
ATTACHMENT.Name = "PK-AS Optic"
--ATTACHMENT.ID = "base" -- normally this is just your filename
ATTACHMENT.Description = { TFA.AttachmentColors["="], "2.8x zoom", TFA.AttachmentColors["="], "5.56mm, 7.62x51mm, 7.62x54mmR", TFA.AttachmentColors["-"], "20% higher zoom time",  TFA.AttachmentColors["-"], "5% slower aimed walking" }
ATTACHMENT.Icon = "entities/saa_PK-AS.png" --Revers to label, please give it an icon though!  This should be the path to a png, like "entities/tfa_ammo_match.png"
ATTACHMENT.ShortName = "PK-AS"
local vpkas = Vector(0, 0, -1)
ATTACHMENT.WeaponTable = {
	...
	["IronSightsPos"] = function( wep, val ) return wep.IronSightsPos_pkas or val end,
	["IronSightsAng"] = function( wep, val ) return wep.IronSightsAng_pkas or val end,
	["IronSightsSensitivity"] = function(wep,val)
		local res = val * wep:Get3DSensitivity( )
		return res, false, true
	end ,
	["Secondary"] = {
		["IronFOV"] = function( wep, val ) return wep.Secondary.IronFOV_pkas or val * 0.7 end,
		["ScopeZoom"] = function( wep, val ) return 4.5 end
	},
	["VMPos"] = function(wep,val ) return wep.VMPos + vpkas or val end,
	["IronSightTime"] = function( wep, val ) return val * 1.26 end,
	["IronSightMoveSpeed"] = function(stat) return stat * 0.95 end,
	["RTScopeFOV"] = function(wep, val) return (wep.RTScopeFOV_pso or wep:GetStat("Secondary.IronFOV")) / wep:GetStat("Secondary.ScopeZoom") / 1.8 end,
	["RTOpaque"] = -1,
...
function ATTACHMENT:Attach(wep)
	...
	wep.RTCode = function( myself , rt, scrw, scrh)
		...
		if not myshad then
			myshad = Material( "models/weapons/optics/PK-AS_Reticle")
		end
