if not ATTACHMENT then
	ATTACHMENT = {}
end

ATTACHMENT.Name = "Kobra EKP-8"
--ATTACHMENT.ID = "base" -- normally this is just your filename
ATTACHMENT.Description = { TFA.AttachmentColors["="], "1x Magnification", TFA.AttachmentColors["-"], "20% higher zoom time" }
ATTACHMENT.Icon = "entities/saa_kobra_d.png" --Revers to label, please give it an icon though!  This should be the path to a png, 	ike "entities/tfa_ammo_match.png"
ATTACHMENT.ShortName = "EKP-8"

ATTACHMENT.WeaponTable = {
	["VElements"] = {
		["scope_kobra"] = {
			["active"] = true
		},
		["scope_kobra_reticlemask"] = {
			["active"] = true
		},
		["scope_kobra_reticle"] = {
			["active"] = true
		},
		["sights_folded"] = {
			["active"] = false
		}
	},
	["WElements"] = {
		["scope_kobra"] = {
			["active"] = true
		},
		["sights_folded"] = {
			["active"] = false
		}
	},
	["IronSightsPos"] = function( wep, val ) return wep.IronSightsPos_kobra or val end,
	["IronSightsAng"] = function( wep, val ) return wep.IronSightsAng_kobra or val end,
	["IronSightTime"] = function( wep, val ) return val * 1.20 end,
	["IronSightMoveSpeed"] = function(stat) return stat * 0.95 end,

}
