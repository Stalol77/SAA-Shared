...
ATTACHMENT.Name = "PSO 1M2-1 4X24 Belomo"
--ATTACHMENT.ID = "base" -- normally this is just your filename
ATTACHMENT.Description = { TFA.AttachmentColors["="], "4x magnification", TFA.AttachmentColors["="], "5.45mm, 7.62mm",  TFA.AttachmentColors["-"], "20% higher zoom time",  TFA.AttachmentColors["-"], "5% slower aimed walking" }
ATTACHMENT.Icon = "entities/saa_pso-1m.png" --Revers to label, please give it an icon though!  This should be the path to a png, like "entities/tfa_ammo_match.png"
ATTACHMENT.ShortName = "PSO-1M2"
local pso1m2vector = Vector(0, 0, -1)
ATTACHMENT.WeaponTable = {
	...
	["VMPos"] = function(wep,val ) return wep.VMPos + pso1m2vector or val end,
	["CrouchPos"] = function(wep,val ) return wep.CrouchPos + 2*pso1m2vector or val end,
	["IronSightsPos"] = function( wep, val ) return wep.IronSightsPos_posp or val end,
	["IronSightsAng"] = function( wep, val ) return wep.IronSightsAng_posp or val end,
	["IronSightsSensitivity"] = function(wep,val)
		local res = val * wep:Get3DSensitivity( )
		return res, false, true
	end ,
	["Secondary"] = {
		["IronFOV"] = function( wep, val ) return wep.Secondary.IronFOV_posp or val * 0.7 end,
	...
	["RTScopeFOV"] = function(wep, val) return (wep.RTScopeFOV_posp or wep:GetStat("Secondary.IronFOV")) / wep:GetStat("Secondary.ScopeZoom") / 4 end,
	["RTOpaque"] = -1,
...
function ATTACHMENT:Attach(wep)
	...
	wep.RTCode = function( myself , rt, scrw, scrh)
		...
		local att, ts
		if wep:VMIV() then
			att = wep.OwnerViewModel:GetAttachment( wep.RTAttachment_posp or 0 )
		end
		...
		if not myshad then
			myshad = Material( "models/weapons/optics/pso1m21")
		end

		local ang = myself.Owner:EyeAngles()
		if wep.ScopeAngleTransforms_posp then
			ang:RotateAroundAxis(ang:Right(), wep.ScopeAngleTransforms_posp.p )
			ang:RotateAroundAxis(ang:Up(), wep.ScopeAngleTransforms_posp.y )
			ang:RotateAroundAxis(ang:Forward(), wep.ScopeAngleTransforms_posp.r )
		end
		cd.angles = ang
		...
		if ts then
			...
			if wep.ScopeOverlayTransforms_posp then
				scrpos.x = scrpos.x + wep.ScopeOverlayTransforms_posp[1]
				scrpos.y = scrpos.y + wep.ScopeOverlayTransforms_posp[2]
			end
			...
			if wep.ScopeOverlayTransformMultiplier_posp then
				scrpos.x = scrpos.x * wep.ScopeOverlayTransformMultiplier_posp
				scrpos.y = scrpos.y * wep.ScopeOverlayTransformMultiplier_posp
			end
			...
			local rtow, rtoh = 0, 0
			if wep.RTScopeOffset_posp then
				rtow = self.RTScopeOffset_posp[1] * rttw
				rtoh = self.RTScopeOffset_posp[2] * rtth
			end
			local rtw, rth = rttw * 1, rtth * 1
			if self.RTScopeScale_posp then
				rtw = rtw * self.RTScopeScale_posp[1]
				rth = rth * self.RTScopeScale_posp[2]
			end
			local distfac = math.pow( 1 - math.Clamp( ( att.Pos:Distance( wep.Owner:GetShootPos() ) - ( wep.ScopeDistanceMin_posp or 4 ) ) / ( wep.ScopeDistanceRange_posp or 12 ), 0, 1 ), 1 )
