...
ATTACHMENT.Name = "1PN-93 Night Vision Scope"
--ATTACHMENT.ID = "base" -- normally this is just your filename
ATTACHMENT.Description = { TFA.AttachmentColors["="], "4x zoom", TFA.AttachmentColors["="], "More like No Vision Scope", TFA.AttachmentColors["="], "5.56mm, 7.62x39mm, 5.45x39mm",  TFA.AttachmentColors["-"], "20% higher zoom time",  TFA.AttachmentColors["-"], "5% slower aimed walking" }
ATTACHMENT.Icon = "entities/saa_1pn93-4.png" --Revers to label, please give it an icon though!  This should be the path to a png, like "entities/tfa_ammo_match.png"
...
ATTACHMENT.WeaponTable = {
	...
	["VMPos"] = Vector(-0.6,-0.2,0.1),
	["IronSightsPos"] = function( wep, val ) return wep.IronSightsPos_1pn93 or val end,
	["IronSightsAng"] = function( wep, val ) return wep.IronSightsAng_1pn93 or val end,
	["IronSightsSensitivity"] = function(wep,val)
		local res = val * wep:Get3DSensitivity( )
		return res, false, true
	end ,
	["Secondary"] = {
		["IronFOV"] = function( wep, val ) return wep.Secondary.IronFOV_1pn93 or val * 0.7 end,
	...
	["RTScopeFOV"] = function(wep, val) return (wep.RTScopeFOV_1pn93 or wep:GetStat("Secondary.IronFOV")) / wep:GetStat("Secondary.ScopeZoom") / 2 end,
	["RTOpaque"] = -1,
...
function ATTACHMENT:Attach(wep)
	...
	wep.RTCode = function( myself , rt, scrw, scrh)
		...
		local att, ts
		if wep:VMIV() then
			att = wep.OwnerViewModel:GetAttachment( wep.RTAttachment_1pn93 or 0 )
		end
		...
		if not myshad then
			myshad = Material( "models/weapons/saa/shared/1pn93_4_off")
		end

		local ang = myself.Owner:EyeAngles()
		if wep.ScopeAngleTransforms_1pn93 then
			ang:RotateAroundAxis(ang:Right(), wep.ScopeAngleTransforms_1pn93.p )
			ang:RotateAroundAxis(ang:Up(), wep.ScopeAngleTransforms_1pn93.y )
			ang:RotateAroundAxis(ang:Forward(), wep.ScopeAngleTransforms_1pn93.r )
		end
		cd.angles = ang
		...
		if ts then
			...
			if wep.ScopeOverlayTransforms_1pn93 then
				scrpos.x = scrpos.x + wep.ScopeOverlayTransforms_1pn93[1]
				scrpos.y = scrpos.y + wep.ScopeOverlayTransforms_1pn93[2]
			end
			...
			if wep.ScopeOverlayTransformMultiplier_1pn93 then
				scrpos.x = scrpos.x * wep.ScopeOverlayTransformMultiplier_1pn93
				scrpos.y = scrpos.y * wep.ScopeOverlayTransformMultiplier_1pn93
			end
			...
			local rtow, rtoh = 0, 0
			if wep.RTScopeOffset_1pn93 then
				rtow = self.RTScopeOffset_1pn93[1] * rttw
				rtoh = self.RTScopeOffset_1pn93[2] * rtth
			end
			local rtw, rth = rttw * 1, rtth * 1
			if self.RTScopeScale_1pn93 then
				rtw = rtw * self.RTScopeScale_1pn93[1]
				rth = rth * self.RTScopeScale_1pn93[2]
			end
			local distfac = math.pow( 1 - math.Clamp( ( att.Pos:Distance( wep.Owner:GetShootPos() ) - ( wep.ScopeDistanceMin_1pn93 or 4 ) ) / ( wep.ScopeDistanceRange_1pn93 or 12 ), 0, 1 ), 1 )
