...
ATTACHMENT.Name = "TA31 Scope"
--ATTACHMENT.ID = "base" -- normally this is just your filename
ATTACHMENT.Description = { TFA.AttachmentColors["="], "3.1x zoom", TFA.AttachmentColors["="], "5.56mm, 7.62x39mm, 5.45x39mm",  TFA.AttachmentColors["-"], "20% higher zoom time",  TFA.AttachmentColors["-"], "5% slower aimed walking" }
ATTACHMENT.Icon = "entities/saa_ta31.png" --Revers to label, please give it an icon though!  This should be the path to a png, like "entities/tfa_ammo_match.png"
...
ATTACHMENT.WeaponTable = {
	...
	["IronSightsPos"] = function( wep, val ) return wep.IronSightsPos_ta31 or val end,
	["IronSightsAng"] = function( wep, val ) return wep.IronSightsAng_ta31 or val end,
	["IronSightsSensitivity"] = function(wep,val)
		local res = val * wep:Get3DSensitivity( )
		return res, false, true
	end ,
	["Secondary"] = {
		["IronFOV"] = function( wep, val ) return wep.Secondary.IronFOV_ta31 or val * 0.7 end,
		["ScopeZoom"] = function( wep, val ) return 3.1 end
	},
	["IronSightTime"] = function( wep, val ) return val * 1.20 end,
	["IronSightMoveSpeed"] = function(stat) return stat * 0.95 end,
	["RTScopeFOV"] = function(wep, val) return (wep.RTScopeFOV_ta31 or wep:GetStat("Secondary.IronFOV")) / wep:GetStat("Secondary.ScopeZoom") / 3.1 end,
	["RTOpaque"] = -1,
...
function ATTACHMENT:Attach(wep)
	...
	wep.RTCode = function( myself , rt, scrw, scrh)
		...
		local att, ts
		if wep:VMIV() then
			att = wep.OwnerViewModel:GetAttachment( wep.RTAttachment_ta31 or 0 )
		end
		...
		if not myshad then
			myshad = Material("models/weapons/optics/ta31_reticule")
		end

		local ang = myself.Owner:EyeAngles()
		if wep.ScopeAngleTransforms_ta31 then
			ang:RotateAroundAxis(ang:Right(), wep.ScopeAngleTransforms_ta31.p )
			ang:RotateAroundAxis(ang:Up(), wep.ScopeAngleTransforms_ta31.y )
			ang:RotateAroundAxis(ang:Forward(), wep.ScopeAngleTransforms_ta31.r )
		end
		cd.angles = ang
		...
		if ts then
			...
			if wep.ScopeOverlayTransforms_ta31 then
				scrpos.x = scrpos.x + wep.ScopeOverlayTransforms_ta31[1]
				scrpos.y = scrpos.y + wep.ScopeOverlayTransforms_ta31[2]
			end
			...
			if wep.ScopeOverlayTransformMultiplier_ta31 then
				scrpos.x = scrpos.x * wep.ScopeOverlayTransformMultiplier_ta31
				scrpos.y = scrpos.y * wep.ScopeOverlayTransformMultiplier_ta31
			end
			...
			local rtow, rtoh = 0, 0
			if wep.RTScopeOffset_ta31 then
				rtow = self.RTScopeOffset_ta31[1] * rttw
				rtoh = self.RTScopeOffset_ta31[2] * rtth
			end
			local rtw, rth = rttw * 1, rtth * 1
			if self.RTScopeScale_ta31 then
				rtw = rtw * self.RTScopeScale_ta31[1]
				rth = rth * self.RTScopeScale_ta31[2]
			end
			local distfac = math.pow( 1 - math.Clamp( ( att.Pos:Distance( wep.Owner:GetShootPos() ) - ( wep.ScopeDistanceMin_ta31 or 4 ) ) / ( wep.ScopeDistanceRange_ta31 or 12 ), 0, 1 ), 1 )
